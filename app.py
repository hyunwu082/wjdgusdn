...
with st.sidebar:
        ...
        weather = st.selectbox("오늘날씨", ["맑음", "흐림", "비/눈", "매우 추움"], key = weather)
        st.markdown("---")
...
with col1:
        st.subheader("상의")
        top_type = st.radio("종류", ["셔츠", "맨투맨", "반팔 티셔츠"], key = top_type)
...
with col2:
        ...
        bottom_color = st.select_slider("핏", options=["슬림", "레귤러", "오버핏"], key = bottom_color)
...
if st.button("코디 완성하기"):
        with st. container(border=True):
                st.subheader(f"{user_name}님의 오늘의 축복")
